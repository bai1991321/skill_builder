  ...
  loginUser: function (req, res) {
    pool.getConnection(function (err, connection) {
      ...
      connection.query(sql, values, function (error, results, fields) {
        ...
        bcrypt.compare(req.body.inputPassword, results[0].user_password, function (err, result) {
          if (result == true) {
            var token = {
              "token": jwt.sign(
  ...
  updateUser: function (userEmail, req, res) {
    pool.getConnection(function (err, connection) {
      ...
      bcrypt.hash(req.body.inputPassword, saltRounds, function (err, hash) {
        ...
        var values = { 'name': req.body.user_name, 'email': req.body.inputEmail, 'password': hash, 'question': req.body.question, 'answer': req.body.answer, 'updatedAt': new Date() }
        // Use the connection
